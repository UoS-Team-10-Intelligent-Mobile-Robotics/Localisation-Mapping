...
class ParticlePathSLAM:
    ...
    def weights_normalisation(self):
        ...
        if sum_weights < 1e-10:
            # If sum is too small, reset all weights to be equal
            for i in range(num_p):
                self.particles[i].weight = 1.0 / num_p
        else:
            # Otherwise, normalize weights
            for i in range(num_p):
                self.particles[i].weight /= sum_weights
    ...
    def resampling(self):
        ...
        if self.number_effective_particles() < self.num_particles * 0.6:
            print("Resampling")
