...
def rigid_body_kinematics(
    mu, u, dt=0.1, mu_gt=None, sigma_motion=Matrix(3, 2), sigma_xy=Matrix(3, 3)
):
    #################################################################################
    ...
    if abs(u_hat[0]) < tol and abs(u_hat[1]) < tol:
        ...
        if abs(u_hat[1]) < tol:
            ...
            H_bb_ = HomogeneousTransformation(
                t_bb_, 0
            )  # heading doesn't change as w=0 (u[1]=0]
            ...
            if np.all(mu_gt != None):
                ...
                # create the homogeneous transformation from b to b',
                H_bb_gt = HomogeneousTransformation(
                    t_bb_gt, 0
                )  # heading doesn't change as w=0 (u[1]=0]
            ...
            t_bc[1] = (
                v / w
            )  # centre of rotation is v/w in the +ve y direction of the body Eq(A1.2.12)
...
class TrajectoryGenerate:
    ...
    def path_to_trajectory(self, v, a):
        ...
        for i in range(len(self.x_path) - 1):
            ...
            P_p2p, Tp_p2p, V_p2p, W_p2p, D_p2p = self._point_to_point(
                self.x_path[i : i + 2],
                self.y_path[i : i + 2],
                [v, a],
                start_stationary,
    ...
    def turning_arcs(self, radius):
        ...
        for i in range(len(self.Tp)):
            ...
            if i == len(self.Tp) - 1 or i == 0:
                delta_angle = 0
            else:
                angle = np.arctan2(
                    self.P[i + 1][1] - self.P[i][1], self.P[i + 1][0] - self.P[i][0]
                )
                angle_prev = np.arctan2(
                    self.P[i][1] - self.P[i - 1][1], self.P[i][0] - self.P[i - 1][0]
                )
            ...
            # calculate the coordinates of the rounded corners
                ...
                straight_distance0 = np.sqrt(
                    (line0_arc_intersect[0] - x_list[1]) ** 2
                    + (line0_arc_intersect[1] - y_list[1]) ** 2
                )
                straight_distance1 = np.sqrt(
                    (line1_arc_intersect[0] - x_list[1]) ** 2
                    + (line1_arc_intersect[1] - y_list[1]) ** 2
                )

                max_allowable0 = np.sqrt(
                    (x_list[0] - x_list[1]) ** 2 + (y_list[0] - y_list[1]) ** 2
                )
                max_allowable1 = np.sqrt(
                    (x_list[2] - x_list[1]) ** 2 + (y_list[2] - y_list[1]) ** 2
                )

                if (
                    straight_distance0 < max_allowable0
                    and straight_distance1 < max_allowable1
                ):
                    ...
                    wp[2] = self.P[i - 1][
                        2
                    ]  # on line0 so should be same angle as incoming line
                    self.P_arc = np.vstack((self.P_arc, wp.T))
                    self.V_arc = np.vstack(
                        (self.V_arc, self.V[i])
                    )  # velocity also needs to be maintained
                    ...
                    if t_bc[1] > 0:
                        w = (
                            self.V[i] / radius
                        )  # if centre is +ve y, turn clockwise at w=v/r
                    else:
                        w = (
                            -self.V[i] / radius
                        )  # if centre is -ve y, turn anticlockwise
                    self.W_arc = np.vstack((self.W_arc, np.array([w])))
                    ...
                    wp[2] = self.P[i][
                        2
                    ]  # on line1 so should be same angle as outgoing line
                    self.P_arc = np.vstack((self.P_arc, wp.T))
                    self.V_arc = np.vstack(
                        (self.V_arc, self.V[i])
                    )  # velocity also needs to be maintained
                    ...
                    t_accumulate += (
                        arc_distance - (straight_distance0 + straight_distance1)
                    ) / self.V[i]
        ...
        # if we are at the end of the data, in which case we sample the last
            ...
            # anything intermediate use extrapolation
                ...
                if np.isnan(Arc[id_T0]).any():
                    ...
                    if turning_arc_flag == False:
                        p[2] = P[id_T0][
                            2
                        ]  # needs to maintain heading until it arrives at the next waypoint for linear
                    else:
                        p[2] = P[id_T1][
                            2
                        ]  # needs to adopt the next headingp[2] = interpolate(t, t0, t1, P[id_T0][2] , P[id_T1][2] , wrap = np.deg2rad(360))
                    ...
                    arc_angle = interpolate(
                        t, t0, t1, gamma_arc0, gamma_arc1, wrap=np.deg2rad(360)
                    )
                    ...
                    p[2] = interpolate(
                        t, t0, t1, P[id_T0][2], P[id_T1][2], wrap=np.deg2rad(360)
                    )
                    ...
                    u[1] = W[id_T0][
                        0
                    ]  # angular velocity is constant along the turning arc

        return p, u

    def wp_progress(
        self, t_robot, p_robot, accept_radius, timeout_factor=2, initial_timeout=30
    ):
        ...
        distance_to_wp = np.sqrt(
            (P[self.wp_id, 0] - p_robot[0]) ** 2 + (P[self.wp_id, 1] - p_robot[1]) ** 2
        )

        # calculate the time spent at on the current waypoint segment
        if self.wp_id == 0:
            segment_time = t_robot
        else:
            segment_time = t_robot - Tp[self.wp_id - 1]
        ...
        # compute delay
        if t_robot > scheduled_time:
            delay = t_robot - scheduled_time
        else:
            delay = 0
        ...
        if self.wp_id == len(Tp) - 1:
            ...
            if distance_to_wp <= accept_radius:
                ...
                print(
                    "Time ",
                    t_robot,
                    " s exceeds Latest allowable arrival",
                    latest_allowable,
                    "s",
                )
                print("************************************************************")
            ...
            if wp_progress_flag == True:
                ...
                if self.wp_id <= len(Tp) - 2:
                    ...
                    if delay > 0:
                        for i in range(self.wp_id - 1, len(Tp)):
                            Tp[i] = Tp[i] + delay
                        if len(self.Tp_arc) == 1:
                            self.Tp = copy.copy(Tp)
                        else:
                            self.Tp_arc = copy.copy(Tp)

    def _point_to_point(
        self, x_points, y_points, params, start_stationary=True, end_stationary=True
    ):
        ...
        if gamma < 0:
            gamma += 2 * np.pi  # wraps angles to be between 0 and 2pi

        # determine straight line distance between points
        total_distance = np.sqrt(
            (y_points[1] - y_points[0]) ** 2 + (x_points[1] - x_points[0]) ** 2
        )
        ...
        if start_stationary == True:
            ...
            P, Tp, V, W, D = self._stack_trajectory(
                P, x_points[0], y_points[0], gamma, Tp, t, V, v, W, 0, D, d
            )

            if end_stationary == True:
                # end of coast phase corresponds to next element of Trajectory matrices
                t += (
                    total_distance - 2 * (0.5 * a * (v / a) ** 2)
                ) / v  # total time - time to accelerate and decelerate
                ...
                P, Tp, V, W, D = self._stack_trajectory(
                    P, x_points[0], y_points[0], gamma, Tp, t, V, 0, W, 0, D, d
                )
    ...
    def _stack_trajectory(
        ...
        D = np.vstack(
            (D, distance_increase)
        )  # we store the distance increment, so subtract the previous distance
...
def kalman_filter_predict(mu, Sigma, u, A, B, R, view_flag=None, x=None, ylim=None):
    ...
    if view_flag is True:
        ...
        if ylim is None:
            ylim = [0, 0.55]  # default, otherwise use what is provided
...
def extended_kalman_filter_predict(
    mu, Sigma, u, f, R, dt, view_flag=False, x=None, xlim=None, ylim=None
):
    """
    Keyword arguments:
    view_flag -- A boolean to show intermediate plots of the prediction and measurement update
    ...
    if view_flag is True:
        if x is None:
            x = np.arange(mu - 10, mu + 10, 0.05)
        if ylim is None:
            ylim = [0, 0.6]  # default, otherwise use what is provided
        if xlim is None:
            xlim = [-5, 5]  # default, otherwise use what is provided
...
def extended_kalman_filter_update(
    mu, Sigma, z, h, Q, view_flag=False, x=None, xlim=None, ylim=None, wrap_index=None
):
    """Keyword arguments:
    view_flag -- A boolean to show intermediate plots of the prediction and measurement update
    optional argument -- a range of states to plot probabilities over, using numpy arrays e.g. np.arange(-8,8,0.05)
    ...
    delta_z = z - pred_z
    if wrap_index != None:
        delta_z[wrap_index] = (delta_z[wrap_index] + np.pi) % (2 * np.pi) - np.pi
    cor_mu = mu + K @ (delta_z)
...
def kde_probability(particles, sigma_resolution=1.0, sampling_resolution=None):
    ...
    if sampling_resolution == None:
        ...
        kde = KernelDensity(kernel="gaussian", bandwidth=sigma_resolution).fit(
            locations.T, sample_weight=particles.weight.T
        )

        state_range_northings = np.linspace(
            min(particles.northings), max(particles.northings), num=sampling_resolution
        )
        state_range_eastings = np.linspace(
            min(particles.eastings), max(particles.eastings), num=sampling_resolution
        )
...
def initialise_particle_distribution(particles, centre=[0, 0], radius=1, heading=0):
    ...
    particles.gamma = np.random.uniform(
        heading - np.deg2rad(10), heading + np.deg2rad(10), particles.N
    )
...
def pf_measurement_probability(particles, measurement):
    ...
    for i in range(particles.N):
        particle_to_measurement = (
            particles.northings[i] - measurement.northings
        ) ** 2 + (particles.eastings[i] - measurement.eastings) ** 2
...
def pf_resample(particles, jitter, verbose=False):
    ...
    if neff(particles) < 0.5:
        ...
        for i in range(particles.N):
            particles.northings[i] = copy.deepcopy(
                particles_copy.northings[indexes[i]]
            ) + radius[i] * np.cos(angle[i])
            particles.eastings[i] = copy.deepcopy(
                particles_copy.eastings[indexes[i]]
            ) + radius[i] * np.sin(angle[i])
...
def discrete_motion_model(particles, gamma, u, dt, process_noise):
    ...
    for i in range(particles.N):
        ...
        if gamma != None:
            p[2] = gamma + np.random.normal(scale=g_std) % (2 * np.pi)
        else:
            p[2] = particles.gamma[i] + np.random.normal(scale=g_std) % (2 * np.pi)
...
class graphslam_frontend:
    ...
    def observation(self, m, sigma, landmark_id, z):
        self.landmark.append(
            copy.copy(m)
        )  # this is need to avoid deep copy behaviour in python
    ...
    def construct_graph(self, visualise_flag=False):
        ...
        for k in range(self.e):
            ...
            if edge_type == "motion":
                ...
                e_ij, A_ij, self.bij = self._Jacobian(
                    edge_type,
                    self.state_vector[3 * i : 3 * i + 3],
                    self.state_vector[3 * j : 3 * j + 3],
                    z_ij,
                    visualise_flag,
                )
                ...
                self.H[3 * i : 3 * i + 3, 3 * i : 3 * i + 3] += (
                    A_ij.T @ Inverse(sigma_ij) @ A_ij
                )
                self.H[3 * i : 3 * i + 3, 3 * j : 3 * j + 3] += (
                    A_ij.T @ Inverse(sigma_ij) @ self.bij
                )
                self.H[3 * j : 3 * j + 3, 3 * i : 3 * i + 3] += (
                    self.bij.T @ Inverse(sigma_ij) @ A_ij
                )
                self.H[3 * j : 3 * j + 3, 3 * j : 3 * j + 3] += (
                    self.bij.T @ Inverse(sigma_ij) @ self.bij
                )

            if edge_type == "landmark":
                # point to correct location in the extended state vector
                self.state_vector[3 * i : 3 * i + 3] = self.pose[i]
                l = 3 * self.n + 2 * self.unique_landmark.index(
                    self.landmark_id_array[j]
                )
                # associate the constraint
                z_il = self.edge[k][3]
                ...
                if np.all(self.state_vector[l : l + 2]) == 0.0:
                    ...
                    e_il, A_il, self.bil = self._Jacobian(
                        edge_type,
                        self.state_vector[3 * i : 3 * i + 3],
                        self.state_vector[l : l + 2],
                        z_il,
                    )
                ...
                self.H[3 * i : 3 * i + 3, 3 * i : 3 * i + 3] += (
                    A_il.T @ Inverse(sigma_il) @ A_il
                )
                self.H[3 * i : 3 * i + 3, l : l + 2] += (
                    A_il.T @ Inverse(sigma_il) @ self.bil
                )
                self.H[l : l + 2, 3 * i : 3 * i + 3] += (
                    self.bil.T @ Inverse(sigma_il) @ A_il
                )
                self.H[l : l + 2, l : l + 2] += (
                    self.bil.T @ Inverse(sigma_il) @ self.bil
                )
    ...
    def _Jacobian(self, edge_type, x_i, x_j, z_ij=None, visualise_flag=False):
        if edge_type == "motion":
            ...
            if visualise_flag == True:
                ...
                print("e_ij[2]", e_ij[2])
                print(
                    "X_j.gamma,X_i.gamma,Z_ij.gamma", X_j.gamma, X_i.gamma, Z_ij.gamma
                )
...
class graphslam_backend:
    ...
    def _update_nodes(self):
        ...
        for i in range(self.n + self.m):
            if i < self.n:
                ...
                indices = [
                    index
                    for index, value in enumerate(self.landmark_id_array)
                    if value == landmark_id
                ]

                for i in range(len(indices)):
                    self.landmark[indices[i]] = self.state_vector[
                        l * 2 + 3 * self.n : l * 2 + 3 * self.n + 2
                    ]
    ...
    def _update_covariance(self):
        ...
        for k in range(self.e):
            ...
            if edge_type == "motion":
                self.pose_covariance[i] = self.sigma[
                    3 * i : 3 * i + 3, 3 * i : 3 * i + 3
                ]
                self.pose_covariance[j] = self.sigma[
                    3 * j : 3 * j + 3, 3 * j : 3 * j + 3
                ]
            ...
            if edge_type == "landmark":
                ...
                for i in range(len(indices)):
                    self.landmark_covariance[indices[i]] = self.sigma[
                        landmark_id * 2 + 3 * self.n : landmark_id * 2 + 3 * self.n + 2,
                        landmark_id * 2 + 3 * self.n : landmark_id * 2 + 3 * self.n + 2,
                    ]
    ...
    def reduce2pose(self, visualise_flag=False):
        ...
        for k in self.unique_landmark:
            ...
            for n in range(len(self.edge)):
                if (
                    self.edge[n][0] == "landmark"
                    and self.landmark_id_array[self.edge[n][2]] == k
                ):
            ...
            i = ij_mod[0]
            H_[i * 3 : i * 3 + 3, i * 3 : i * 3 + 3] -= (
                H_[i * 3 : i * 3 + 3, 2 * k + l : 2 * k + l + 2]
                @ sigma_m[k * 2 : k * 2 + 2, k * 2 : k * 2 + 2]
                @ H_[2 * k + l : 2 * k + l + 2, i * 3 : i * 3 + 3]
            )

            if len(ij_mod) > 1:
                j = ij_mod[1]

                H_[i * 3 : i * 3 + 3, j * 3 : j * 3 + 3] -= (
                    H_[i * 3 : i * 3 + 3, 2 * k + l : 2 * k + l + 2]
                    @ sigma_m[k * 2 : k * 2 + 2, k * 2 : k * 2 + 2]
                    @ H_[2 * k + l : 2 * k + l + 2, j * 3 : j * 3 + 3]
                )
                H_[j * 3 : j * 3 + 3, i * 3 : i * 3 + 3] -= (
                    H_[j * 3 : j * 3 + 3, 2 * k + l : 2 * k + l + 2]
                    @ sigma_m[k * 2 : k * 2 + 2, k * 2 : k * 2 + 2]
                    @ H_[2 * k + l : 2 * k + l + 2, i * 3 : i * 3 + 3]
                )
                H_[j * 3 : j * 3 + 3, j * 3 : j * 3 + 3] -= (
                    H_[j * 3 : j * 3 + 3, 2 * k + l : 2 * k + l + 2]
                    @ sigma_m[k * 2 : k * 2 + 2, k * 2 : k * 2 + 2]
                    @ H_[2 * k + l : 2 * k + l + 2, j * 3 : j * 3 + 3]
                )
        ...
        if visualise_flag == True:
            ...
            show_information(
                ...
                pose_graph.m,
                2,
                matrix_compare=self.b[0 : self.n * 3],
            )
            ...
            show_information(
                ...
                pose_graph.m,
                2,
                matrix_compare=self.H[0 : self.n * 3, 0 : self.n * 3],
            )
...
class ParticleMap:
    ...
    def initialise(self):
        ...
        self.gamma = np.random.normal(self.gamma, self.auxilliary_noise[0]) % (
            2 * np.pi
        )
    ...
    def predict(self, u, dt=1):
        ...
        p[2] = self.gamma + np.random.normal(scale=self.auxilliary_noise[0]) % (
            2 * np.pi
        )
...
class ParticlePathSLAM:
    ...
    def gp_weight_update(
        ...
        past_observations_rb_in_range = past_observations_rb[
            (past_observations_rb[:, 0] > sensor_range[0])
            & (past_observations_rb[:, 0] < sensor_range[1])
        ]
        ...
        sum_squared_stds = std_prediction**2 + (new_obs_std) ** 2
        ...
        if show_plots == True:
            ...
            ax.set_title(
                "Weight is:" + str(round(mean_weight, 3)) + " before normalisation"
            )
            plt.show()
    ...
    def resampling(self):
        ...
        print(
            "Proportion of effective particles: ",
            self.number_effective_particles() / (self.num_particles),
        )
    ...
    def show_particles(self, kde_pose=None, show_all=False):
        ...
        if np.any(kde_pose) != None:
            plt.scatter(
                kde_pose[1], kde_pose[0], s=100, marker="+", c="k", label="kde mean"
            )
        ...
        if show_all == True:
            ...
            plt.plot(
                np.rad2deg(particle_gamma),
                np.zeros(len(particle_gamma)),
                "r|",
                label="particles",
            )
            ...
            if np.any(kde_pose) != None:
                plt.plot(
                    np.rad2deg(kde_pose[2]),
                    0,
                    "k+",
                    markersize=12,
                    label="mean particles",
                )
...
def lidar_scan(p_eb, environment_map, lidar, sigma_observe):
    ...
    for i in range(len(environment_map)):
        ...
        z_lm, sigma_rtheta, t_lm, sigma_xy = lidar.loc_to_rangeangle(
            p_eb, t_em, sigma_observe
        )
        m_range.append(z_lm[0])
        m_bearing.append(z_lm[1])

    # Sampling the map
    bearing_resolution = np.linspace(
        -lidar.scan_fov / 2, lidar.scan_fov / 2, lidar.n_beams
    )
    ...
    for idx, theta in enumerate(bearing_resolution):
        if not np.all(np.isnan(m_bearing)):
            i = np.nanargmin(
                abs(theta - np.array(m_bearing))
            )  # Find nearest valid map element

            if abs(theta - m_bearing[i]) < lidar.scan_fov / (
                2 * lidar.n_beams
            ):  # Check threshold
                noisy_range = m_range[i] + np.random.normal(
                    0, m_range[i] * sigma_observe[0, 0], 1
                )
