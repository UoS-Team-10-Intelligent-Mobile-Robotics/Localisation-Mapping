    ...
    def infinite_loop(self):
        ...
        if aruco_pose is not None:
            ...
            if self.initialise_pose == True:
                ...
                # particle path SLAM initialised
                p_init = Vector(3)
                p_init[0] = self.measured_pose_northings_m
                p_init[1] = self.measured_pose_eastings_m
                p_init[2] = self.measured_pose_yaw_rad
                self.particle_filter = ParticlePathSLAM(
                    self.num_particles,
                    self.lidar,
                    p_init,
