...
class LaptopPilot:
    def __init__(self, simulation):
        ...
        self.lidar_data = None  # x and y in the e-frame
        ...
        self.z_lm = Vector(2) # lidar measurement in the map
        self.t_em = Vector(2)  # lidar measurement in the e-frame
        self.observations_std = Vector(2)  
        ...
        self.x_bl = 0.1
        self.y_bl = 0  #location of lidar centre in b-frame secondary axis

        self.lidar = RangeAngleKinematics(self.x_bl, self.y_bl, distance_range = [0.1, 1], scan_fov = np.deg2rad(90), n_beams = 30) #Task
        self.position_std = 0.2  # initial spread, resampling spread
        self.gpr_sample_cap = 30
        self.num_particles = 300  # Task
        self.initial_position_std = 0.1  # Task
        self.auxilliary_noise = [np.deg2rad(5), 0.01, np.deg2rad(3)]  # Task
        self.num_particles = 30  # Task

        ### measurement noise model
        self.sigma_observe = Matrix(2,2)
        self.sigma_observe[0,0] = 0.1 #1% of range
        self.sigma_observe[0,1] = 0
        self.sigma_observe[1,0] = np.deg2rad(1) #1 degree per metre range
        self.sigma_observe[1,1] = 0

        self.map_std = self.sigma_observe[0,0]*self.lidar.distance_range[1] #max range equivalent

        self.length_scale= 0.2 # GP modelling parameter
        ...
        param_name = f"{log_int_float_convert(self.tau_s)}_{log_int_float_convert(self.L)}"

        # param_name = f"{log_int_float_convert(self.tau_s)}_{log_int_float_convert(self.L)}_{log_int_float_convert(self.NE_std[0])}_{log_int_float_convert(self.G_std[0])}_{log_int_float_convert(self.DOTX_std[0])}_{log_int_float_convert(self.DOTG_std[0])}_{log_int_float_convert(self.R_N[0])}_{log_int_float_convert(self.R_E[0])}_{log_int_float_convert(self.R_G[0])}_{log_int_float_convert(self.R_DOTX[0])}_{log_int_float_convert(self.R_DOTG[0])}"
        self.datalog = DataLogger(log_dir="logs", prefix=param_name)
    ...
    #     """Update the visualization with current particles and robot position"""
        
    #     # Get particle positions
    #     if hasattr(self, 'particle_filter') and self.particle_filter is not None:
    #         particles = self.particle_filter.particles
    #         particle_positions_x = [p.pose[1] for p in particles]  # Eastings
    #         particle_positions_y = [p.pose[0] for p in particles]  # Northings
            
    #         # Update particle scatter plot
    #         self.particle_scatter.set_offsets(np.column_stack([particle_positions_x, particle_positions_y]))
            
    #         # Update robot position (estimated from particles)
    #         kde_pose, _ = self.particle_filter.kde_pose()
    #         self.robot_position.set_offsets([[kde_pose[1], kde_pose[0]]])
            
    #         # Update map points if lidar data exists
    #         if self.lidar_data is not None and len(self.lidar_data) > 0:
    #             self.map_points.set_offsets(self.lidar_data[:, [1, 0]])  # [eastings, northings]
            
    #         # Update path
    #         if self.path is not None:
    #             path_x = self.eastings_path
    #             path_y = self.northings_path
    #             self.path_line.set_data(path_x, path_y)
            
    #         # Update plot limits if necessary
    #         self.ax.set_xlim(min(min(particle_positions_x)-0.5, 0), max(max(particle_positions_x)+0.5, 2))
    #         self.ax.set_ylim(min(min(particle_positions_y)-0.5, 0), max(max(particle_positions_y)+0.5, 2))
    ...
    def lidar_callback(self, msg):
        ...
        self.got_new_measuremet = True 
        self.m_bearing = []  # Reset the bearing list for each new scan
        self.z_range_std = np.full(self.lidar.n_beams, np.nan)
        ...
        num_beams_to_process = min(len(msg.ranges), self.lidar.n_beams)
        # print(len(msg.ranges), "lidar beams received")
        # print("Number of beams to process:", self.lidar.n_beams)
        # print("Lidar beams to process:", num_beams_to_process)
        # m to e frame
        self.lidar_data = np.zeros((len(msg.ranges), 2))  # specify length of the lidar data

        # for each map measurement
        for i in range(len(msg.ranges)):
            self.z_lm[0,0] = msg.ranges[i]
            self.z_lm[1,0] = msg.angles[i]
            self.m_bearing.append(self.z_lm[1,0])   

            self.t_em = self.lidar.rangeangle_to_loc(p_eb, self.z_lm)  #output x and y in the e-frame

            self.lidar_data[i, 0] = self.t_em[0]  
            self.lidar_data[i, 1] = self.t_em[1]

        self.bearing_resolution = np.linspace(-self.lidar.scan_fov / 2, self.lidar.scan_fov / 2, self.lidar.n_beams)
        
        for idx, theta in enumerate(self.bearing_resolution):        
            if not np.all(np.isnan(self.m_bearing)):
                i = np.nanargmin(abs(theta - np.array(self.m_bearing)))  # Find nearest valid map element
                
                if abs(theta - self.m_bearing[i]) < self.lidar.scan_fov / (2 * self.lidar.n_beams):  # Check threshold
                    self.z_range_std[i] = abs(self.z_lm[0] * self.sigma_observe[0, 0])  

        # this filters out any NaN values
        self.lidar_data = self.lidar_data[~np.isnan(self.lidar_data).any(axis=1)]

        self.observations_std = np.column_stack((self.z_range_std, self.bearing_resolution))
    ...
    def generate_trajectory(self):
        ...
        if self.relative_path == True: #may need to wait as it has reached the first point
    ...
    def infinite_loop(self):
        ...
        if aruco_pose is not None:
            ...
            if self.initialise_pose == True:
                ...
                p[0] = self.measured_pose_northings_m
                p[1] = self.measured_pose_eastings_m
                p[2] = self.measured_pose_yaw_rad

                self.particle_filter = ParticlePathSLAM(self.num_particles,
                                                self.lidar,
                                                p, 
                                                self.position_std,                                 
                                                self.auxilliary_noise)
                kde_pose, kde_std = self.particle_filter.kde_pose(sigma_resolution=0.2, sampling_resolution=100)
                self.resampling_flag = False  # False, True
                ...
                # self.particle_scatter = self.ax.scatter([], [], alpha=0.3, color='blue', s=5, label='Particles')
                # self.robot_position = self.ax.scatter([], [], color='red', s=100, marker='*', label='Robot Estimate')
                # self.map_points = self.ax.scatter([], [], color='green', s=10, label='Map Points')
                # self.path_line, = self.ax.plot([], [], 'k--', label='Planned Path')
        ...
        if self.initialise_pose != True:
            ...
            p_robot[2,0] = p_robot[2,0] % (2 * np.pi)  # deal with angle wrapping
            ...
            self.path.path_to_trajectory(self.velocity, self.acceleration) #velocity and acceleration
            self.path.turning_arcs(self.turning_radius) #turning radius
            self.path.wp_progress(self.t, p_robot, self.acceptance_radius) #probot heading in rad
            p_ref, u_ref = self.path.p_u_sample(self.t)  # output rad 
            ...
            # total control
            u = u_ref + du  # combine the feedforward and feedback control twist components
            ...
            self.particle_filter.predict(u,dt) # motion of particles       
            print('Particle filter predict')     
            # self.update_plot()  # Update visualization after prediction

            if self.got_new_measuremet == True:
                print('Parrticle filter update')
                


                self.particle_filter.observation_update(self.lidar_data, self.observations_std, self.map_std, self.length_scale, resampling_flag = True, gpr_sample_cap = self.gpr_sample_cap,show_plot=True )
                
                # print("Lidar data shape:", self.lidar_data.shape)
                # print("Observations std shape:", observations_std.shape)
